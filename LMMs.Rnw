...
Example: SR/OR relative clause data from English (Grodner and Gibson 2005, Expt 1). 
...
dat$so<-ifelse(dat$condition=="objgap",1,-1)
...
\item find the appropriate transform for the data
...
<<fig.height=3>>=
car::qqPlot(residuals(m0.lmer))
@

\end{frame}

\begin{frame}[fragile]\frametitle{Model assumptions, model selection}
\framesubtitle{Box-Cox transform}

<<fig.height=3>>=
MASS::boxcox(lm(rawRT~so,dat))
@
...
<<fig.height=3>>=
m1.lmer<-lmer(I(-1000/rawRT)~so + (1+so|subject)+(1+so||item),dat)
summary(m1.lmer)$coefficients
@
...
<<fig.height=3>>=
car::qqPlot(residuals(m1.lmer))
@

\end{frame}

\begin{frame}[fragile]\frametitle{Model assumptions, model selection}
\framesubtitle{Use likelihood ratio test to choose the most parsimonious model}

Example:

<<>>>=
mNULL<-lmer(logrt~so + (1+so||subject)+(1+so||item),dat)
m<-lmer(logrt~so + (1+so|subject)+(1+so||item),dat)
anova(mNULL,m)
@
...
\begin{frame}[fragile]\frametitle{Model assumptions, model selection}
\framesubtitle{Additionally, simulate data to check parameter recovery}

See my textbook draft for code and discussion.

\end{frame}

\begin{frame}[fragile]\frametitle{Model assumptions, model selection}
\framesubtitle{NHST for the fixed effect}

\small
<<>>=
mNULL<-lmer(logrt~1+so + (1+so|subject)+(1+so||item),dat)
m<-lmer(logrt~1 + (1+so|subject)+(1+so||item),dat)
anova(mNULL,m)
@
...
\small
<<>>=
mNULL<-lmer(I(-1000/rawRT)~1+so + (1+so|subject)+(1+so||item),dat)
m<-lmer(I(-1000/rawRT)~1 + (1+so|subject)+(1+so||item),dat)
anova(mNULL,m)
@

\end{frame}

\begin{frame}[fragile]\frametitle{For more details}

https://vasishth.github.io/Freq\_CogSci/
